...
void showRankings(const char* mode) {
    ...
    if (strcmp(mode, "single") == 0) {
        getScore = [](Player p) { return p.singleScore; };
    } else if (strcmp(mode, "quick") == 0) {
        getScore = [](Player p) { return p.quickScore; };
    } else if (strcmp(mode, "tournament") == 0) {
        getScore = [](Player p) { return p.tournamentScore; };
    } else {
        getScore = [](Player p) { return p.endlessScore; };
    }

    for (int i = 0; i < numPlayers - 1; i++) {
        for (int j = 0; j < numPlayers - i - 1; j++) {
            if (getScore(sortedPlayers[j]) < getScore(sortedPlayers[j + 1])) {
    ...
    printf("\nRankings for %s mode:\n", mode);
    int highScore = getScore(sortedPlayers[0]);
    for (int i = 0; i < numPlayers; i++) {
        int score = getScore(sortedPlayers[i]);
